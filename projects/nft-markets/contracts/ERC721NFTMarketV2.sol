...
contract ERC721NFTMarketV2 is Initializable, ERC721HolderUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     * @param _WHSKAddress: WHSK address
    ...
    function initialize(
        ...
        require(_WHSKAddress != address(0), "Operations: WHSK address cannot be zero");
    ...
    /**
     * @notice Buy token with HSK by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     */
    function buyTokenUsingHSK(address _collection, uint256 _tokenId) external payable nonReentrant {
        // Wrap HSK
        IWETH(WHSK).deposit{value: msg.value}();

        _buyToken(_collection, _tokenId, msg.value, true);
    }

    /**
     * @notice Buy token with WHSK by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     * @param _price: price (must be equal to the askPrice set by the seller)
     */
    function buyTokenUsingWHSK(address _collection, uint256 _tokenId, uint256 _price) external nonReentrant {
        IERC20(WHSK).safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function claimPendingRevenue() external nonReentrant {
        ...
        IERC20(WHSK).safeTransfer(address(msg.sender), revenueToClaim);
    ...
    function recoverFungibleTokens(address _token) external onlyOwner {
        require(_token != WHSK, "Operations: Cannot recover WHSK");
     ...
     * @param _withHSK: whether the token is bought with HSK (true) or WHSK (false)
     */
    function _buyToken(address _collection, uint256 _tokenId, uint256 _price, bool _withHSK) internal {
        ...
        // Transfer WHSK
        IERC20(WHSK).safeTransfer(askOrder.seller, netPrice);
        ...
        // Emit event
        emit Trade(_collection, _tokenId, askOrder.seller, msg.sender, _price, netPrice, _withHSK);
    }
